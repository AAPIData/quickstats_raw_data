...
#first of set up the grabber function
  lep_grabber <- function(geo, year){
    ...
    #load data for lep
    print("Getting African American Alone")
    table1 <- get_acs(table = "B16005B", geography = geo, year = year,summary_var ="B16005B_001" )
    # print("Getting African American Alone")
    # table2 <- get_acs(table = "B16005C", geography = geo, year = year)
    table3 <- get_acs(table = "B16005D", geography = geo, year = year,summary_var ="B16005D_001" )
    table4 <- get_acs(table = "B16005E", geography = geo, year = year,summary_var ="B16005E_001" )
    table5 <- get_acs(table = "B16005H", geography = geo, year = year,summary_var ="B16005H_001" )
    table6 <- get_acs(table = "B16005I", geography = geo, year = year,summary_var ="B16005I_001" )
    ...
    #merge into one
    table <- rbind(table1,table3, table4, table5, table6)
    table <- table %>% 
      select(NAME, group, variable, estimate,moe,summary_est) %>% 
      rename(`Group Population`=summary_est) %>% 
      left_join(label) %>% select(label, everything()) # join table
    
    tbl_lep <- table %>%
      separate(variable, into=c("prefix","ending"),sep="_") %>% 
      filter(ending %in% c("006","011")) %>% 
